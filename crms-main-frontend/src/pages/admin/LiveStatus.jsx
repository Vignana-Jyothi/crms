...
export default function LiveStatus() {
  ...
  function refresh() {
    ...
    // Now uses backend default

    resourcesApi
      .liveStatus(selectedDate, startTimeParam, endTimeParam)
      .then((data) => {
        setRooms(data);
  ...
  // Auto-expand accordions when filtering
  useEffect(() => {
    setCollapsedDepts({});
  }, [searchTerm, selectedDept, selectedBlock, selectedAvailability]);
  ...
  const filteredRooms = rooms.filter(r => {
    ...
    return matchesSearch && matchesDept && matchesBlock && matchesAvailability;
  });
  ...
  // Get unique departments and blocks for the dropdown
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          />
          <div className="text-left xl:text-right flex flex-col xl:items-end whitespace-nowrap w-full xl:w-auto mt-2 xl:mt-0">
            <p className="text-[10px] text-ink/50 mb-1">Updated: {lastUpdated.toLocaleTimeString()}</p>
            <button
              onClick={refresh}
              disabled={loading}
              className="rounded bg-white border border-line px-3 py-1.5 text-xs font-medium text-navy hover:bg-paper w-max"
            >
      ...
      <div className="mt-8 space-y-8">
        {Object.entries(grouped).sort().map(([dept, deptRooms]) => {
          const forceOpen = searchTerm.length > 0 || selectedDept !== 'All' || selectedBlock !== 'All';
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {deptRooms.sort((a,b) => a.resourceName.localeCompare(b.resourceName)).map((r) => (
                      ...
                      >
                        ...
                        {r.capacity && <p className="mt-1 text-xs font-semibold text-ink/40 uppercase tracking-wider">Capacity: {r.capacity}</p>}
                      </button>
                    ))}
                  </div>
                </div>
      ...
      {selectedRoom && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className={`p-6 border-b ${selectedRoom.isFree ? 'bg-forest-light border-forest/20' : 'bg-brick-light border-brick/20'}`}>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {selectedRoom.capacity && <p className="text-xs font-semibold text-ink/40 uppercase tracking-wider mt-2">Capacity: {selectedRoom.capacity} People</p>}
                </div>
            ...
            <div className="p-4 bg-paper/50 border-t border-line flex items-center justify-end gap-3">
              <button 
                onClick={() => setSelectedRoom(null)}
                className="px-5 py-2 text-ink/70 hover:text-navy text-sm font-semibold transition-colors"
              >
                Close
              </button>
              {selectedRoom.isFree && (
                <a 
                  href={`/resources/${selectedRoom.resourceId}`}
