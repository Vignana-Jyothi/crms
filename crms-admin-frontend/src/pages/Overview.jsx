  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl font-semibold text-navy">Welcome back, {user?.name.split(' ')[0]}</h1>
          <p className="mt-1 text-sm text-ink/60">Here is what is happening across your resources today.</p>
        </div>
        <Link to="/live" className="rounded bg-navy px-4 py-2 text-sm font-semibold text-white hover:bg-navy-dark transition-colors shadow-sm">
          View Live Status
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-10">
            <StatCard label="Pending Approvals" value={stats.pending ?? '0'} to="/approvals" color={stats.pending > 0 ? 'amber' : 'navy'} />
            <StatCard label="Approved Bookings" value={stats.activeBookings ?? '0'} to="/bookings" color="forest" />
            <StatCard
              label="Total Resources"
              value={stats.totalResources ?? '0'}
              to={canManageResources ? '/resources' : undefined}
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="rounded-xl border border-line bg-white shadow-sm overflow-hidden flex flex-col">
              <div className="border-b border-line bg-paper/50 px-6 py-4 flex items-center justify-between">
                ...
                <Link to="/approvals" className="text-xs font-semibold text-navy hover:underline">View all</Link>
              </div>
                    ...
                    {pendingApprovals.map(a => (
                      <li key={a.approvalId} className="p-4 hover:bg-paper/30 transition-colors">
                        <div className="flex justify-between items-start">
                          ...
                          <Link to="/approvals" className="text-xs bg-amber/20 text-amber-dark font-medium px-2 py-1 rounded">
                            Review
                          </Link>
                        </div>
            ...
            <div className="rounded-xl border border-line bg-white shadow-sm overflow-hidden flex flex-col">
              <div className="border-b border-line bg-paper/50 px-6 py-4 flex items-center justify-between">
                ...
                <Link to="/bookings" className="text-xs font-semibold text-navy hover:underline">View all</Link>
              </div>
              <div className="p-0 flex-1">
