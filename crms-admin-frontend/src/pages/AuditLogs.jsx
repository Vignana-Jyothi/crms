          ...
          {filteredLogs.map((l) => (
            <li key={l.auditId} className="flex items-start gap-4 px-4 py-3 text-sm hover:bg-paper/30">
              ...
              <span className="w-48 shrink-0">
                <span className="block font-medium text-ink">{l.user?.name || `User #${l.userId}`}</span>
                {l.user?.department?.departmentName && (
                  <span className="block text-xs text-ink/50 mt-0.5">{l.user.department.departmentName}</span>
                )}
              </span>
